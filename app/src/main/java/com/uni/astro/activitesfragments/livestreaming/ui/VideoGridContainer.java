package com.uni.astro.activitesfragments.livestreaming.ui;
...
public class VideoGridContainer extends RelativeLayout implements Runnable {
    ...
    public void addUserVideoSurface(int uid, SurfaceView surface) {
        ...
        int id = -1;
        if(uid==userID){
            if (mUidList.contains(userID)) {
                mUidList.remove((Integer) userID);
                mUserViewList.remove((Integer) userID);
            }
    ...
    public void removeUserVideo(int uid) {

        if (uid==userID && mUidList.contains(uid)) {
            mUidList.remove((Integer) uid);
            mUserViewList.remove(0);
