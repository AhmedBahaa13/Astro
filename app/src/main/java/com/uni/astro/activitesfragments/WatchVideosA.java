...
import com.uni.astro.activitesfragments.profile.creatorplaylist.CreatePlaylistStepTwoF;
import com.uni.astro.activitesfragments.profile.creatorplaylist.ShowPlaylistF;
import com.uni.astro.models.CreatePlaylistModel;
...
public class WatchVideosA extends AppCompatLocaleActivity implements FragmentCallBack {
    ...
    public void callVideoApi() {
        ...
        if (whereFrom.equalsIgnoreCase("repostVideo")) {
    ...
    public void parseRepostVideoData(String responce) {
    ...
    // api for get the videos list from server
    private void callApiForRepostVideos() {
        JSONObject parameters = new JSONObject();
        try {
            parameters.put("user_id", userId);
            parameters.put("starting_point", "" + pageCount);
        ...
        VolleyRequest.JsonPostRequest(WatchVideosA.this, ApiLinks.showUserRepostedVideos, parameters,Functions.getHeaders(this), new Callback() {
            @Override
            public void onResponce(String resp) {
                swiperefresh.setRefreshing(false);
